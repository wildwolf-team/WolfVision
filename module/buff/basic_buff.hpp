...
// TODO(fqjun) ：能量机关类 继承抽象类
class RM_Buff {
 public:
  // 初始化参数结构体
  RM_Buff() = default;
  explicit RM_Buff(const std::string& _buff_config_address);
  ~RM_Buff();
   ...
   */
  void
  runTask(cv::Mat& _input_img, uart::Receive_Data& _receive_info, uart::Write_Data& _send_info);
   ...
   */
  uart::Write_Data runTask(cv::Mat& _input_img, uart::Receive_Data& _receive_info);
 ...
 private:
   ...
   */
  void Input(cv::Mat& _input_img, const int& _my_color);
 ...
 private:
  ...
  // 内轮廓
  fan_armor::FanArmor small_target_;
  // 外轮廓
  abstract_blade::FanBlade big_target_;

  // 当前检测得到的打击目标（遍历使用）
  abstract_target::Target candidated_target_;
  // 当前检测打击目标（现在）
  abstract_target::Target current_target_;
  // 打击目标队列
 ...
 private:
  ...
  basic_pnp::PnP buff_pnp_ = basic_pnp::PnP(
      "/home/jun/workplace/Github/RM_code/RM_2020_vision_code/Basis/RM_VideoCapture/cameraParams/"
      "cameraParams_1333.xml",
      "/home/jun/workplace/Github/RM_code/RM_2020_vision_code/Basis/angle_solve/pnp_config.xml");
 ...
 private:
  // 帧率测试
  fps::RM_FPS buff_fps_1_{"Part 1"};
  fps::RM_FPS buff_fps_2_{"Part 2"};
  fps::RM_FPS buff_fps_;  // 计算时间
};

inline void RM_Buff::Input(cv::Mat& _input_img, const int& _my_color) {
