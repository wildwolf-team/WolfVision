...
class Target {
 public:
  ...
  void inputParams(const abstract_blade::FanBlade& _fan_blade,
                   const fan_armor::FanArmor&      _fan_armor);
...
inline abstract_blade::FanBlade Target::Blade() { return this->fan_blade_; }

inline fan_armor::FanArmor Target::Armor() { return this->fan_armor_; }

inline float Target::Angle() { return this->angle_; }

inline float Target::diffAngle() { return this->diff_angle_; }

inline abstract_object::ObjectType Target::Type() { return this->type_; }

inline double Target::areaRatio() { return this->area_ratio_; }

inline std::vector<cv::Point2f> Target::vector2DPoint() { return this->points_2d_; }

inline cv::Point2f Target::vector2DPoint(const int& _i) { return this->points_2d_[_i]; }

inline cv::Point2f Target::deltaPoint() { return this->points_2d_[0] - this->points_2d_[3]; }
Target::Target() {
  fan_blade_        = abstract_blade::FanBlade();
  fan_armor_        = fan_armor::FanArmor();
  this->angle_      = 0.f;
  this->diff_angle_ = 0.f;
  this->type_       = abstract_object::UNKNOW;
...
void Target::updateVertex(cv::Mat& _img) {
  ...
  float up_distance =
      abstract_object::centerDistance(point_up_center, this->fan_blade_.Rect().center);
  float down_distance =
      abstract_object::centerDistance(point_down_center, this->fan_blade_.Rect().center);
...
void Target::setType(const abstract_object::ObjectType& _type) { this->type_ = _type; }

void Target::setType(cv::Mat& _bin_img) {
  // 上层中心点 和 下层中心点
  cv::Point2f point_up_center   = (this->fan_armor_.Vertex(0) + this->fan_armor_.Vertex(1)) * 0.5;
  cv::Point2f point_down_center = (this->fan_armor_.Vertex(2) + this->fan_armor_.Vertex(3)) * 0.5;
  ...
  if (left_intensity <= 15 && right_intensity <= 15) {
    this->type_ = abstract_object::INACTION;
  } else {
    this->type_ = abstract_object::ACTION;
  }
}

void Target::displayInactionTarget(cv::Mat& _img) {
  if (this->type_ == abstract_object::INACTION) {
    for (int k = 0; k < 4; ++k) {
